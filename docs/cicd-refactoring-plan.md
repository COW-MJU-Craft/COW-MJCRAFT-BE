...
- 자동 롤백·canary — 현 규모에 과잉. **수동 롤백 절차는 [`docs/rollback.md`](rollback.md)로 이행 완료 (2026-09-02).** 자동 롤백은 계속 도입하지 않는다 — "직전 정상 SHA" 판단이 틀리면 상황이 더 나빠진다
- ~~staging~~ — **2026-08 결정 번복.** API 계약을 깨는 변경을 프론트와 맞춰 사전 검증할 환경이 필요해져 `develop` 브랜치 + Coolify `development` 환경을 도입했다 (#151). 이 문서 작성 시점(2026-07-11)의 "현 규모에 과잉" 판단은 더 이상 유효하지 않다
- 보안 경로 승인 게이트 — 솔로 운영이라 승인 0명 유지 (2인 이상 되면 CODEOWNERS + 승인 1명). 재검토 시점 추적: #168
