...
public interface AdminOrderExportControllerDocs {
    ...
    @Operation(
            summary = "프로젝트별 주문 XLSX 다운로드",
            description = "선택한 프로젝트의 주문 상세 정보를 XLSX로 다운로드합니다. 날짜는 둘 다 생략하거나 둘 다 입력해야 합니다."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "XLSX 다운로드 성공",
                    content = @Content(
    ...
    @Operation(
            summary = "날짜별 주문 XLSX 다운로드",
            description = "모든 프로젝트에서 지정한 날짜 범위에 생성된 주문 상세 정보를 XLSX로 다운로드합니다."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
